//
//  CBContainerTransitionContext.swift
//  Livestar.swift
//
...
let CBContainerTransitionEndNotification = "livestar.Notification.ContainerTransitionEnd"
let CBInteractionEndNotification = "livestar.Notification.InteractionEnd"

class CBContainerTransitionContext: NSObject, UIViewControllerContextTransitioning {
    ...
    func finishInteractiveTransition() {
        ...
        let displayLink = CADisplayLink(target: self, selector: #selector(CBContainerTransitionContext.finishChangeButtonAppear(_:)))
        displayLink.add(to: RunLoop.main, forMode: RunLoopMode.defaultRunLoopMode)
        
        //当 CBScrollTabBarViewController 作为一个子 VC 内嵌在其他容器 VC 内，比如 NavigationController 里时，在 CBScrollTabBarViewController 内完成一次交互转场后
        ...
        perform(#selector(CBContainerTransitionContext.fixBeginTimeBug), with: nil, afterDelay: remainingTime)
    }
    
    func cancelInteractiveTransition() {
        isInteractive = false
        isCancelled = true
        let displayLink = CADisplayLink(target: self, selector: #selector(CBContainerTransitionContext.reverseCurrentAnimation(_:)))
        displayLink.add(to: RunLoop.main, forMode: RunLoopMode.defaultRunLoopMode)
        NotificationCenter.default.post(name: Notification.Name(rawValue: CBInteractionEndNotification), object: self)
    }
    ...
    unowned fileprivate var privateContainerViewController: CBContainerViewController
    ...
    init(containerViewController: CBContainerViewController, containerView: UIView, fromViewController fromVC: UIViewController, toViewController toVC: UIViewController) {
    ...
    fileprivate func transitionEnd() {
        ...
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: CBContainerTransitionEndNotification), object: self)
    }
    ...
    @objc fileprivate func reverseCurrentAnimation(_ displayLink: CADisplayLink) {
        ...
        if timeOffset > 0 {
            ...
            perform(#selector(CBContainerTransitionContext.removeFakeFromView(_:)), with: fakeFromView, afterDelay: 1/60)
        }
    }
